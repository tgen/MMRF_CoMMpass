...
##############################
##
## Require applications, inactivate if available in your path
##
##############################
...
# Input BAM file, this can be removed to run in batch mode
#INPUT_BAM=MMRF_1179_1_mDup.bam
...
NON_BCELL_CONTAMINATION_GENELIST=${BUILD_DIR}Non_Bcell_Contamination_GeneList.txt
...
then
	...
	grep -w "IG_C_gene" Immunoglobulin_Positions.txt | awk '{OFS="\t" ; if($1==14) print $11}' | sort | uniq > IgH_Constant_Genes.txt
	grep -w "IG_V_gene" Immunoglobulin_Positions.txt | awk '{OFS="\t" ; if($1==14) print $11}' | sort | uniq > IgH_Variable_Genes.txt
	grep -w "IG_C_gene" Immunoglobulin_Positions.txt | awk '{OFS="\t" ; if($1==2) print $11}' | sort | uniq > IgK_Constant_Genes.txt
	grep -w "IG_V_gene" Immunoglobulin_Positions.txt | awk '{OFS="\t" ; if($1==2) print $11}' | sort | uniq > IgK_Variable_Genes.txt
	grep -w "IG_C_gene" Immunoglobulin_Positions.txt | awk '{OFS="\t" ; if($1==22) print $11}' | sort | uniq > IgL_Constant_Genes.txt
	grep -w "IG_V_gene" Immunoglobulin_Positions.txt | awk '{OFS="\t" ; if($1==22) print $11}' | sort | uniq > IgL_Variable_Genes.txt
	grep "IG_" ${GTF} | grep -w "exon" | grep -v "pseudogene" | grep "IG_C_gene" | awk '{OFS="\t" ; if($1==14) print $0}' > Immunoglobulin_Regions.gtf
	grep "IG_" ${GTF} | grep -w "exon" | grep -v "pseudogene" | grep "IG_C_gene" | awk '{OFS="\t" ; if($1==2) print $0}' >> Immunoglobulin_Regions.gtf
	grep "IG_" ${GTF} | grep -w "exon" | grep -v "pseudogene" | grep "IG_C_gene" | awk '{OFS="\t" ; if($1==22) print $0}' >> Immunoglobulin_Regions.gtf
	grep "IG_" ${GTF} | grep -w "exon" | grep -v "pseudogene" | grep "IG_V_gene" | awk '{OFS="\t" ; if($1==14) print $0}' >> Immunoglobulin_Regions.gtf
	grep "IG_" ${GTF} | grep -w "exon" | grep -v "pseudogene" | grep "IG_V_gene" | awk '{OFS="\t" ; if($1==2) print $0}' >> Immunoglobulin_Regions.gtf
